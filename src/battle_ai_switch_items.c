...
static bool32 ShouldSwitchIfHasBadOdds(u32 battler)
{
    ...
    for (i = 0; i < MAX_SELECTABLE_MOVES; i++)
    {
        ...
        if (aiMove != MOVE_NONE)
        {
            ...
            || aiMoveEffect == EFFECT_SPIKES || aiMoveEffect == EFFECT_TOXIC_SPIKES || aiMoveEffect == EFFECT_SHOCK_TRAP || aiMoveEffect == EFFECT_STEALTH_ROCK || aiMoveEffect == EFFECT_STICKY_WEB || aiMoveEffect == EFFECT_LEECH_SEED
